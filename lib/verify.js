...
async function verify_proof(input, folderPath)
{
    try {
        ...
        verifierOnchain = true;
        result = {
          verifierOnchain,
          txnResponse,
          verfied: publicSignals,
        };

        return result;
    }catch (error) {
        return {
            verifierOnchain: false,
            error: error.message || "An error occurred during proof verification."
        }
    }
}
